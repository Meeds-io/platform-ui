...
public class UIFormColorPicker extends UIFormInputBase<String> {
    ...
    /*
     * final public UIFormColorPicker setColors(List<SelectItemOption<String>> options) { options_ = options ; if(options_ ==
     * null || options_.size() < 1) return this; value_ = options_.get(0).getValue(); return this ; }
     */
        ...
        while (i <= size() / items()) {
            ...
            while (j <= items() && count < size()) {
                ...
                w.write("<a href=\""
                        ...
                        + "\" class=\""
                        + color.getName()
                        + " ColorCell \" onmousedown=\"event.cancelBubble=true\"></a>");
                count++;
                j++;
            }
            w.write("</div>");
    ...
    public static class Colors {
        ...
        public static final Color[] COLORS = { O_POWDERBLUE, O_ORCHID, O_PALEGOLDENROD, O_PALEGREEN, O_OLIVE, O_OLIVEDRAB,
                O_ORANGERED, O_PALETURQUOISE, O_PALEVIOLETRED, O_PAPAYAWHIP, O_PEACHPUFF, O_PERU, O_PINK, O_PLUM, O_PURPLE,
                O_RED, O_ROSYBROWN, O_ROYALBLUE, O_SADDLEBROWN, O_SALMON, O_SANDYBROWN, O_SEAGREEN, O_SEASHELL, O_SIANNA,
                O_SILVER, O_SKYBLUE, O_THISTLE, O_TOMATO, O_TURQUOISE, O_VIOLET, O_WHEAT, O_YELLOW };

        public static final String[] COLORNAMES = { N_POWDERBLUE, N_ORCHID, N_PALEGOLDENROD, N_PALEGREEN, N_OLIVE, N_OLIVEDRAB,
                N_ORANGERED, N_PALETURQUOISE, N_PALEVIOLETRED, N_PAPAYAWHIP, N_PEACHPUFF, N_PERU, N_PINK, N_PLUM, N_PURPLE,
                N_RED, N_ROSYBROWN, N_ROYALBLUE, N_SADDLEBROWN, N_SALMON, N_SANDYBROWN, N_SEAGREEN, N_SEASHELL, N_SIANNA,
                N_SILVER, N_SKYBLUE, N_THISTLE, N_TOMATO, N_TURQUOISE, N_VIOLET, N_WHEAT, N_YELLOW };

        public static final String[] CODES = { H_POWDERBLUE, H_ORCHID, H_PALEGOLDENROD, H_PALEGREEN, H_OLIVE, H_OLIVEDRAB,
                H_ORANGERED, H_PALETURQUOISE, H_PALEVIOLETRED, H_PAPAYAWHIP, H_PEACHPUFF, H_PERU, H_PINK, H_PLUM, H_PURPLE,
                H_RED, H_ROSYBROWN, H_ROYALBLUE, H_SADDLEBROWN, H_SALMON, H_SANDYBROWN, H_SEAGREEN, H_SEASHELL, H_SIANNA,
                H_SILVER, H_SKYBLUE, H_THISTLE, H_TOMATO, H_TURQUOISE, H_VIOLET, H_WHEAT, H_YELLOW };
        ...
        public static class Color {
            ...
            public void setCode(String code) {
                code_ = code;
                /*
                 * R = Integer.parseInt(code.substring(1,2), 16) ; G = Integer.parseInt(code.substring(3,2), 16) ; B =
                 * Integer.parseInt(code.substring(5,2), 16) ;
                 */
            }
        }
